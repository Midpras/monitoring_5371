<script>
    ...
    function mergeBreakdown(snapshots, worker) {
        ...
        snapshots.forEach((snapshot) => {
            ...
            (workerData?.rows || []).forEach((row) => {
                const current = rows.get(row.kode_subsls);
                if (!current) rows.set(row.kode_subsls, { ...row, recent: [] });
                else rows.set(row.kode_subsls, { ...current, ...row, recent: current.recent });
    ...
    function progressValue(row, snapshotDate) {
        ...
        return !entry ? '-' : value(entry.cumulative);
    }
    function progressNote(row, snapshotDate) {
        const entry = recentFor(row, snapshotDate);
        return !entry ? '' : `Δ ${signed(entry.daily)}`;
    }
    ...
    function chartMax() { return Math.max(...series.flatMap((point) => [Math.abs(point.daily_ppl || 0), Math.abs(point.daily_pml || 0)]), 1); }
    function barHeight(metric) { return metric == null ? 3 : Math.max(4, Math.abs(metric) / chartMax() * 100); }
    function deadlineNote() {
        return summary?.deadline?.date ? displayDate(summary.deadline.date) : 'Belum diatur';
    }
    function deadlineStatus() {
        if (!summary?.deadline?.date) return 'Atur dari halaman admin';
        if (summary.deadline.status === 'overdue') return 'Tenggat terlewati';
        if (summary.deadline.status === 'due') return 'Hari terakhir';
        return `${summary.deadline.days_remaining} hari tersisa`;
    }
    function requiredLabel() {
        if (summary?.deadline?.status === 'overdue' || summary?.deadline?.status === 'due') return 'Harus selesai';
        return 'Kebutuhan per hari';
    }
    function paceGap(type) {
        const required = summary?.metrics?.[`required_daily_${type}`];
        const actual = summary?.metrics?.[`net_change_${type}`];
        return required == null || actual == null ? null : actual - required;
    }
...
<main class="dashboard-shell dashboard-v2">
    ...
    {:else if summary?.snapshot}
        ...
        <section class="chart-grid dashboard-chart-grid">
            <article class="data-panel daily-chart-panel"><div class="panel-heading"><div><span>PROGRES HARIAN</span><h2>Perubahan semua snapshot</h2></div><small>{series.length ? `${series.length} tanggal` : 'Belum ada data'}</small></div>
                {#if series.length}<div class="daily-bars" aria-label="Grafik progres harian PPL dan PML">{#each series as point}<div class="daily-bar-group"><div class="daily-bar-pair"><div class:negative-bar={isNegative(point.daily_ppl)} class="daily-bar" style={`height:${barHeight(point.daily_ppl)}%`} title={`PPL ${chartLabel(point, 'ppl')}`}></div><div class:negative-bar={isNegative(point.daily_pml)} class="daily-bar pml-bar" style={`height:${barHeight(point.daily_pml)}%`} title={`PML ${chartLabel(point, 'pml')}`}></div></div><strong>{point.date.slice(5)}</strong><small>PPL {chartLabel(point, 'ppl')}</small><small>PML {chartLabel(point, 'pml')}</small></div>{/each}</div><div class="legend"><span><i class="ppl-dot"></i>PPL</span><span><i class="pml-dot"></i>PML</span></div>{:else}<p class="chart-empty">Belum ada riwayat snapshot.</p>{/if}
            </article>
            <article class="data-panel chart-summary deadline-summary"><div class="panel-heading"><div><span>TENGGAT</span><h2>Batas penyelesaian</h2></div></div><div class="chart-summary-value"><strong>{deadlineNote()}</strong><span>{deadlineStatus()}</span></div><dl><div><dt>Target PPL / hari</dt><dd>{value(summary.metrics.required_daily_ppl)}</dd><small>Gap {signed(paceGap('ppl'))}</small></div><div><dt>Target PML / hari</dt><dd>{value(summary.metrics.required_daily_pml)}</dd><small>Gap {signed(paceGap('pml'))}</small></div></dl></article>
        </section>
